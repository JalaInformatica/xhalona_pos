...
class MasterMasAllScreen extends StatelessWidget {
            ...
            children: [
              mButton(() {
                Navigator.of(context).pushAndRemoveUntil(
                    ...
                    (route) => false);
              }, "Add Master All", Icons.add, double.infinity),
              SizedBox(
                height: 5.h,
              ),
              Obx(() => Expanded(
                    ...
                    data:
                      ...
                      return [
                        AppTableCell(
                            value: masAll.groupMId,
                            index: i,
                            onEdit: () {
                              goTo(context, masAll);
                            },
                            onDelete: () async {
                              await messageHapus(masAll.rowId, masAll.masterId,
                                  masAll.masDesc);
                            },
                            showOptionsOnTap: true),
                        AppTableCell(
                            value: masAll.masterId,
                            index: i,
                            onEdit: () {
                              goTo(context, masAll);
                            },
                        ...
                        AppTableCell(
                          ...
                          onDelete: () async {
                            await messageHapus(
                                masAll.rowId, masAll.masterId, masAll.masDesc);
                          },
  ...
  Future<dynamic> messageHapus(int rowId, String masId, String namaRekening) {
    return SmartDialog.show(builder: (context) {
      return AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                String result = await _masAllRepository.deleteMasAll(
                    rowId: rowId, masId: masId);
  ...
  Future<dynamic> goTo(BuildContext context, MasAllDAO pekerjaan) {
    return Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(
            builder: (context) => AddEditMasAll(pekerjaan: pekerjaan)),
        (route) => false);
  }
}
